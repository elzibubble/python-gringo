...
struct Statement : Printable, Locatable {
    using SplitVec = std::vector<std::pair<ULit, UBodyAggrVec>>;
    enum class Type { RULE, EXTERNAL, WEAKCONSTRAINT };
    Statement(UHeadAggr &&head, UBodyAggrVec &&body, bool external);
    Statement(UTerm &&weight, UTerm &&prioriy, UTermVec &&tuple, UBodyAggrVec &&body);
    Statement(UTermVec &&tuple, UBodyAggrVec &&body);
    Statement(UHeadAggr &&head, UBodyAggrVec &&body, Type type);
