...
void Program::rewrite(Defines &defs) {
    for (auto &block : blocks_) {
        ...
        auto replace = [&](Defines &defs, Value fact) -> Value {
            ...
            if (rt) {
                Location loc{rt->loc()};
                block.addedStms.emplace_back(make_locatable<Statement>(loc, make_unique<SimpleHeadLiteral>(make_locatable<PredicateLiteral>(loc, NAF::POS, std::move(rt))), UBodyAggrVec{}, false));
                return Value();
            }
        ...
        for (auto &x : splits) {
            ...
            block.stms.emplace_back(make_locatable<Statement>(
                loc,
...
Ground::Program Program::toGround(PredDomMap &domains) {
    ...
    for (auto &x : undef) {
        GRINGO_REPORT(W_ATOM_UNDEFINED)
            << x.first << ": warning: atom is undefined:\n"
            << "  " << *x.second << "\n";
    }
    return prg;
}
