    ...
    if (!--blocked) {
        ...
        for (auto &x : import) { 
            ...
            if (!x.empty()) {
                ...
                // NOTE: it would be much better to do this at the end of instantiation where it is sure that the marks are empty
                head.first.exports.offset = head.first.exports.size();
            }
        }
    }
    for (auto &head : heads) {
...
void Rule::report(Output::OutputBase &out) {
    if (external) {
        if (defines) {
            ...
            out.external(*std::get<0>(ret), TruthValue::False);
        }
    }
    else {
